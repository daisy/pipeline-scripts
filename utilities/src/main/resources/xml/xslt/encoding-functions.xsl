...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="brl:unicode-braille-to-escape-sequence" as="xs:string">
        ...
        <xsl:sequence select="my:dec-to-hex(string-to-codepoints($unicode-braille))"/>
    </xsl:function>
    ...
    <xsl:function name="my:dec-to-hex" as="xs:string">
        ...
        <xsl:sequence select="if ($dec = 0) then '0' else concat(if ($dec > 16) then my:dec-to-hex($dec idiv 16) else '',
