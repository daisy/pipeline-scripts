...
<xsl:stylesheet xmlns="" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="#all" version="2.0"
    ...
    <xsl:template match="*:smil">
        <smil xml:base="{base-uri(.)}">
            <xsl:variable name="meta-duration" select="pf:mediaoverlay-clock-value-to-seconds(head/meta[@name='ncc:timeInThisSmil']/@content)"/>
            ...
            <xsl:attribute name="calculated-duration"
                select="sum(for $audio in (body//audio[@clip-begin or @clip-end]) return (($audio/pf:mediaoverlay-clock-value-to-seconds(@clip-end),$meta-duration)[1] - $audio/pf:mediaoverlay-clock-value-to-seconds(@clip-begin)))"/>
            ...
            <xsl:attribute name="meta-totalTime" select="pf:mediaoverlay-clock-value-to-seconds(head/meta[@name='ncc:totalElapsedTime']/@content)"/>
        </smil>
    </xsl:template>
