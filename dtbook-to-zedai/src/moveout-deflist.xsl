...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="main">
        ...
        <xsl:call-template name="test-and-move">
            ...
            <xsl:with-param name="target-elem-name" select="'dl'" tunnel="yes"/>
            <xsl:with-param name="valid-parents-list" select="tokenize('annotation,prodnote,sidebar,address,covertitle,div,epigraph,imggroup,caption,code-block,kbd,li,note,img,blockquote,level,level1,level2,level3,level4,level5,level6,td,th,poem,samp', ',')"  tunnel="yes"/>
