...
<p:declare-step version="1.0" name="dtbook-to-zedai.convert" type="px:dtbook-to-zedai-convert" xmlns:p="http://www.w3.org/ns/xproc"
    ...
    <p:input port="in-memory.in" sequence="true">
        <p:documentation>
            ...
            <xd:detail>
                <xd:p>One or more DTBook documents to be transformed. In the case of multiple documents, a merge will be performed.</xd:p>
                <xd:p>While all resources are referenced in the fileset on the `fileset` output port, the `in-memory`-port can contain pre-loaded documents so
                    that they won't have to be loaded from disk. This means that the input documents does not have to be stored to disk - they could have been
                    generated by a preceding conversion, allowing for easy chaining of scripts.</xd:p>
            </xd:detail>
        </p:documentation>
    </p:input>
    ...
    <p:output port="fileset.out" primary="true">
        <p:documentation>
            ...
            <xd:detail>Contains references to the DTBook file and any resources it references (images etc.). For each file that is not stored in memory, the
                xml:base is set with an absolute URI pointing to the location on disk where it is stored. This lets the href reflect the path and filename of
                the resulting resource without having to store it. This is useful for chaining conversions.</xd:detail>
        </p:documentation>
            ...
            <p:choose>
                <p:when test="/*/@media-type = 'application/x-dtbook+xml'">
                    ...
                    <p:split-sequence name="dtbook-input.for-each.split">
                        ...
                        <p:with-option name="test" select="concat('/*/@xml:base = &quot;',$dtbook-base,'&quot;')"/>
                    </p:split-sequence>
                    <p:count/>
                    <p:choose>
                        <p:when test=". &gt; 0">
                        ...
                        <p:otherwise>
                            <cx:message>
                                <p:with-option name="message" select="concat('Input DTBook not in memory, loading from disk: ', $dtbook-base)"/>
                            </cx:message>
                            <p:load>
        ...
        <p:choose>
            <p:when test=". = 0">
                <p:error xmlns:err="http://www.w3.org/ns/xproc-error" code="PEDZ00">
                    ...
                    <p:input port="source">
                        <p:inline>
                            <message>No XML documents with the DTBook media type ('application/x-dtbook+xml') found in the fileset.</message>
                        </p:inline>
                    </p:input>
                </p:error>
                <p:sink/>
    ...
    <p:xslt name="generate-css">
        ...
        <p:input port="stylesheet">
            <p:inline>
                ...
                <xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:tmp="http://www.daisy.org/ns/pipeline/tmp">
                    ...
                    <p:inline>
                        ...
                        <xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
                            ...
                            <xsl:template match="/">
                                ...
                                <xsl:processing-instruction name="xml-stylesheet"> href="<xsl:value-of select="$css"/>" </xsl:processing-instruction>
                                <xsl:apply-templates/>
                            </xsl:template>
                            <!-- identity template -->
    ...
    <p:documentation>Insert the MODS file reference metadata into the head of ZedAI</p:documentation>
    ...
    <p:group name="result.fileset">
        <p:output port="result"/>
        <p:variable name="dtbook-base" select="replace(p:resolve-uri(/*/@href,/*/@xml:base),'^(.*/)[^/]*$','$1')"/>
